...
let
  ...
  swayidleConfig = ''
    ...
    } '${lib.optionalString graphicsProfileCfg.allowSuspend ''notify-send -r 999 -h byte:urgency:1 -t 5000 -a "System" "The session will lock soon due to inactivity";''} brightnessctl -q -s; brightnessctl -q -m | { IFS=',' read -r _ _ _ brightness _ && [ "''${brightness%\%}" -le 25 ] || brightnessctl -q set 25% ;}' resume "brightnessctl -q -r || brightnessctl -q set 100%"
