...
let
  ...
  tmp = pkgs.makeDesktopItem {
    name = "com.system76.CosmicSettings.Network";
    desktopName = "Network & Wireless";
    comment = "Manage network connections";
    type = "Application";
    exec = "env DBUS_SYSTEM_BUS_ADDRESS=unix:path=/tmp/dbusproxy_net.sock cosmic-settings network";
    terminal = false;
    categories = [ "COSMIC" ];
    keywords = [ "COSMIC" ];
    noDisplay = true;
